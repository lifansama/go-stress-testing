...
压测完成后会自动生成HTML格式的测试报告（默认输出到 `report.html`），报告包含以下内容：

#### 4.6.1 AI智能评分

报告会对压测结果进行智能评分（满分100分），评分维度包括：

| 指标 | 满分 | 评分标准 |
|:-----|:-----|:---------|
| 成功率 | 30分 | 100%=30分, 99%=27分, 95%=20分, <90%=0分 |
| QPS吞吐 | 25分 | 根据并发效率动态计算 |
| 响应速度 | 20分 | <50ms=20分, <100ms=18分, <500ms=12分 |
| 稳定性(TP99) | 15分 | TP99与平均时间的比值评估 |
| 错误码 | 10分 | 仅成功码=10分, 有4xx=-3分, 有5xx=-5分 |

**评级标准**: A(90+), B(80-89), C(70-79), D(60-69), F(<60)

支持外部AI API（OpenAI兼容格式）获取更智能的优化建议：
...
| 图表类型 | 说明 |
|:---------|:-----|
| 响应时间分布 | 柱状图展示请求响应时间的分布情况 |
| 状态码分布 | 饼图展示各HTTP状态码的占比 |
| QPS随时间变化 | 折线图展示测试过程中QPS的变化趋势 |
| 成功率随时间变化 | 折线图展示成功率的变化趋势 |
| 接口耗时随时间变化 | 折线图展示平均/最大/最小响应时间变化 |
| 错误码随时间变化 | 折线图展示各状态码数量的变化趋势 |

时间轴格式：`2024-01-07 13:30:09`（年-月-日 时:分:秒）

#### 4.6.3 使用示例

```bash
# 默认生成HTML报告（report.html）
./go-stress-testing -c 10 -n 100 -u https://www.baidu.com/

# 指定输出路径
./go-stress-testing -c 10 -n 100 -u https://www.baidu.com/ -o result.html

# 生成Markdown格式报告
./go-stress-testing -c 10 -n 100 -u https://www.baidu.com/ -o report.md -format md

# 自定义成功状态码（如201）
./go-stress-testing -c 10 -n 100 -u https://api.example.com/create -code 201
```
...
HTML报告包含以下部分：
- **AI评分卡片**: 显示总分、评级和各项得分
- **测试信息**: 压测地址、并发数、请求数、开始/结束时间
- **测试结果摘要**: 成功/失败数、成功率、QPS、下载字节数
- **响应时间统计**: 最小/最大/平均响应时间、TP90/TP95/TP99
- **可视化图表**: 6种交互式图表
- **状态码分布**: 各状态码出现次数和占比
- **时间序列数据**: 详细的时间序列数据表格
...
| UI界面 | 无         | 有              | 有               | 无                            | 无                   |
| 优势   | 使用简单，上手简单 | 支持分布式、压测数据支持导出 | 插件丰富，支持生成HTML报告 | 项目开源，使用简单，没有依赖，支持webSocket压测，支持HTML报告和AI评分 | 更加真实的模拟用户，支持更高的压测力度 |
